...
def custom_transform(size=100):
    ...
    resizer = MyResizer(size)

    train_transform = transforms.Compose(
        [
            transforms.RandomCrop(100, padding=15, pad_if_needed=False, fill=0, padding_mode='constant'),
            transforms.RandomResizedCrop(size, scale = (2.0, 3.2),interpolation=2),
